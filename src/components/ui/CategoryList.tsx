...
export default function CategoryList({ categories }: Props) {
  ...
  return (
    <section
      className="flex flex-col gap-8 w-full max-w-[1040px] max-lg:max-w-[700px] max-md:max-w-[343px] mx-auto overflow-hidden mt-20 mb-40 max-md:my-16"
      ref={containerRef}
    >
      <div className="relative w-full max-lg:max-w-[700px] max-md:max-w-[343px] m-auto">
        <h2 className="text-2xl font-medium pl-2">Browse By Category</h2>
        <button
          onClick={prevPage}
          aria-label="Previous"
            ...
            <div className="grid grid-cols-6 max-lg:grid-cols-3 max-md:grid-cols-2 gap-8 max-md:gap-4 w-full m-auto">
              {padPage(page, perPage).map((category, i) => (
                <Link
                  ...
                  key={
                    (category && (category.id ?? category.url_icon)) ??
                    `empty-${i}`
                  }
                >
                  {category ? (
